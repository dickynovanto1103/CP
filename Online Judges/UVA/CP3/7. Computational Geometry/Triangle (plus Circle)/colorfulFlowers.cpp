...
int main() {
	...
	while(scanf("%lf %lf %lf",&a,&b,&c)!=EOF){
		double rBesar = rCircumCircle(a,b,c);
		double rKecil = rInCircle(a,b,c);
		double luasLingBesar = pi*rBesar*rBesar;
		double luasLingKecil = pi*rKecil*rKecil;
		double luasSegitiga = area(a,b,c);
		double sun = luasLingBesar - luasSegitiga;
		double violet = luasSegitiga - luasLingKecil;
		double roses = luasLingKecil;
		printf("%.4lf %.4lf %.4lf\n",sun,violet,roses);
	}
	return 0;
}