...
int main() {
	...
	double minimx = min(p1.x,p2.x), maksx = max(p1.x,p2.x);
	double minimy = min(p1.y,p2.y), maksy = max(p1.y,p2.y);
	//printf("titik potong:\n");
	//printf("%.9lf %.9lf\n",c.x,c.y);
	if(c.x>=minimx && c.x<=maksx && c.y>=minimy && c.y<=maksy){
		minimx = min(p3.x,p4.x), maksx = max(p3.x,p4.x);
		minimy = min(p3.y,p4.y), maksy = max(p3.y,p4.y);
		if(c.x>=minimx && c.x<=maksx && c.y>=minimy && c.y<=maksy){
			printf("%.9lf %.9lf\n",c.x,c.y);	
		}else{
			printf("-1 -1\n");	
