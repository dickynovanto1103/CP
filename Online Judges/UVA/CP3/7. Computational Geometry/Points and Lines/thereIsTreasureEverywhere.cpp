...
int main() {
	...
	while(cin>>kata && kata!="END"){
		...
		double x = 0.0, y = 0.0;
		string angka, arah;
		int i, j, n = kata.length();
		for(i=0;i<n-1;i++){//karena idx n-1 adalah titik
			//cari angka
			angka = "";
			for(j=i;j<n;j++){
				int bil = kata[j]-48;
				if(bil>=0 && bil<=9){angka+=kata[j];}
				else{break;}
			}
			arah = "";
			int idx = j;
			for(j=idx;j<n;j++){
				if(kata[j]==',' || kata[j]=='.'){break;}
				else{arah+=kata[j];}
			}
			int sum = stringToInt(angka);
			double hipot = sum/sqrt(2);
			if(arah=="N"){y+=sum;}
			else if(arah=="E"){x+=sum;}
			else if(arah=="W"){x-=sum;}
			else if(arah=="S"){y-=sum;}
			else if(arah=="NW"){
				x-=hipot; y+=hipot;
			}else if(arah=="NE"){
				x+=hipot; y+=hipot;
