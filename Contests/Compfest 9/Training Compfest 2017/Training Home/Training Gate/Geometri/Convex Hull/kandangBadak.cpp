...
int main() {
	...
	P.pb(P[0]);
	vector<point> P1 = CH(P);
	int ukuran = P1.size();
	/*for(i=0;i<PAns.size();i++){
		printf("%lf %lf\n",PAns[i].x,PAns[i].y);
	}*/
	P1.pop_back();//buang elemen point terakhir biar ga duplicate
	int idx = 0;
	for(i=1;i<ukuran;i++){
		if(P1[i].x < P1[idx].x || (P1[i].x==P1[idx].x && P1[i].y < P1[idx].y)){
			idx = i;
		}
	}
	ukuran = P1.size();
	/*printf("convex hull\n");
	for(i=0;i<ukuran;i++){
		printf("%d %d\n",(int)P1[i].x,(int)P1[i].y);
	}
	printf("idx: %d\n",idx);*/
	
	for(i=idx;i<ukuran;i++){
		//printf("yand dicetak\n");
		printf("%d %d\n",(int)P1[i].x,(int)P1[i].y);
	}
	//printf("done\n");
