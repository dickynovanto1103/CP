...
int main() {
	...
	while(scanf("%lf %lf %lf %lf",&l,&w,&h,&teta)!=EOF){
		double tot = l*w*h;
		double miring = hypot(l,h);
		double batasSudut = radToDeg(asin(h/miring));

		//printf("batas sudut: %lf\n",batasSudut);
		if(teta>batasSudut){
			double sudut2Rad = degToRad(90-teta);
			double sudutRad = degToRad(teta);
			double sisi2 = (h/sin(sudutRad))*sin(sudut2Rad);
			double sisi1 = h;
			double hipot = hypot(sisi1,sisi2);
			double luas = area(sisi1,sisi2,hipot);
			double volum = luas*w;
			printf("%.3lf mL\n",volum);
		}else{
			//printf("masuk sini:\n");
			double sudut2Rad = degToRad(90-teta);
			double sudutRad = degToRad(teta);
			//printf("sudut2Rad: %lf sudutRad: %lf\n",sudut2Rad,sudutRad);
			double sisi2 = (l/sin(sudut2Rad))*sin(sudutRad);
			double sisi1 = l;
			double hipot = hypot(sisi1,sisi2);
			//printf("sisi1: %lf sisi2: %lf hipot: %lf\n",sisi1,sisi2,hipot);
			double luas = area(sisi1,sisi2,hipot);
			//printf("luas: %lf\n",luas);
			double volum = luas*w;
			//printf("tot: %lf volum: %lf\n",tot,volum);
			printf("%.3lf mL\n",tot-volum);	
		}
