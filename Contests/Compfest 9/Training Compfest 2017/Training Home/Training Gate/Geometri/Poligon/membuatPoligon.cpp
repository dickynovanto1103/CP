...
int main() {
	...
	vector<point> P;
	scanf("%d",&n);
	//double minimx = inf,minimy = inf,maksx=-inf,maksy = -inf;
	double xtengah=0.0, ytengah=-.0;
	for(i=0;i<n;i++){
		double a,b;
	...
	int idx = 0;
	for(i=1;i<n;i++){
		if(P[i].y < P[idx].y || (fabs(P[i].y - P[idx].y)<eps && P[i].x > P[idx].x)){idx = i;}
	}
	point temp = P[0]; P[0] = P[idx]; P[idx] = temp;//swap routine

	pivot = P[0];
	//printf("xtengah: %lf ytengah: %lf\n",xtengah,ytengah);
