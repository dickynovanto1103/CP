        ...
        if(c==0){
            ...
            if(kiri){
                for(int i=0;i<permen[lantai].size();++i){
                    ...
                    temp = min(temp, (2*permen[lantai][i].fi) + 1 + solve(lantai+1, kiri, max(c - candy, 0)));
                }
            }
            else{
                for(int i=permen[lantai].size()-1;i>=0;--i){
                    candy += permen[lantai][i].se;
                    temp = min(temp, 2*(m+1-permen[lantai][i].fi) + 1 + solve(lantai+1, kiri, max(c - candy, 0)));
                }
            }
            temp = min(temp, m+1 + solve(lantai+1, 1-kiri, max(c - candy, 0)));
            dp[lantai][kiri][c] = min(1 + solve(lantai+1, kiri, c), temp);
        }
    }
