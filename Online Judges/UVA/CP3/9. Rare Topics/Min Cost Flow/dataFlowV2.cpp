...
int V,E,pre[MAX_V],last[MAX_V];
edge_type flowVal,flowCost,cap[MAX_E],cost[MAX_E],to[MAX_E],ne[MAX_E],dist[MAX_V],pot[MAX_V];
...
void add_edge(int u, int v, edge_type capacity, edge_type cst){
	...
	cost[E] = cst, ne[E] = last[u];
	last[u] = E++;
	
	to[E] = u, cap[E] = 0;
	cost[E] = -cst, ne[E] = last[v];
	last[v] = E++;
}
...
void mcmf(int s, int t){
	...
	while(true){
		...
		for(int i = t;i!=s;i = to[pre[i]^1]) f = min(f,cap[pre[i]]);
		...
		for(int i = 0;i<V;++i) if (pre[i]!=-1) pot[i] += dist[i];
	}
}
