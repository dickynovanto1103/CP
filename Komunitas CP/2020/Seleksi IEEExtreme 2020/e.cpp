#include <bits/stdc++.h>

using namespace std;
#define inf 1000000000
#define unvisited -1
#define visited 1
#define eps 1e-9
#define mp make_pair
#define pb push_back
#define pi acos(-1.0)
#define uint64 unsigned long long
#define FastSlowInput ios_base::sync_with_stdio(false); cin.tie(NULL); cout.tie(NULL);
#define debug if(true)
typedef long long ll;
typedef vector<int> vi;
typedef pair<int,int> ii;
typedef pair<int,ii> iii;
typedef vector<ii> vii;
typedef vector<iii> viii;

struct edge
{
	int a, b, w, idx;
};

edge edges[200001];

class UnionFind{
private: 
	vi rank,p,setSize;
	int numset,i;
public:
	UnionFind(int n){
		numset=n; setSize.assign(n,1); rank.assign(n,0); p.assign(n,0);
		for(i=0;i<n;i++){p[i]=i;}
	}

	int findSet(int i){return (p[i]==i) ? i : (p[i]=findSet(p[i]));}
	bool isSameSet(int i, int j){return findSet(i)==findSet(j);}
	void unionSet(int i, int j){
		if(!isSameSet(i,j)){
			numset--;
			int x=findSet(i), y=findSet(j);
			if(rank[x] > rank[y]){p[y]=x; setSize[x]+=setSize[y];}
			else{
				p[x]=y;
				setSize[y]+=setSize[x];
				if(rank[x]==rank[y]){rank[y]++;}
			}
		}
	}
	int numDisjointSet(){return numset;}
	int sizeSetOf(int i){return setSize[findSet(i)];}
};

bool cmp(edge a, edge b) {
	return a.w < b.w;
}

int main(){
	int n,m,i,j;
	scanf("%d %d",&n,&m);

	for(i=0;i<m;i++){
		int a,b,w;

		scanf("%d %d %d",&edges[i].a, &edges[i].b, &edges[i].w);
		edges[i].a--; edges[i].b--;
		edges[i].idx = i+1;
	}
	sort(edges, edges + m, cmp);
	UnionFind UF(n);
	ll sum = 0;
	int maksW = 0;
	map<int,int> mapper;
	vi listEdgeMst;
	for(i=0;i<m;i++){
		int w = edges[i].w;
		int a = edges[i].a, b = edges[i].b;
		maksW = max(maksW, w);
		if(!UF.isSameSet(a,b)) {
			UF.unionSet(a,b);
			listEdgeMst.pb(edges[i].idx);
			if(UF.numDisjointSet() == 1){
				break;
			}
		}
	}

	if(i == m || i == m-1){
		printf("%d\n",m);
		for(i=1;i<=m;i++){
			printf("%d\n",i);
		}
	}else{ //pasti kena break
		if(edges[i+1].w == maksW) {
			//print semua edge yang ga sampe w
			vi listAns;
			for(i=0;i<m;i++){
				int w = edges[i].w;
				int idx = edges[i].idx;
				if(w == maksW){break;}
				listAns.pb(idx);
			}
			sort(listAns.begin(), listAns.end());
			int sz = listAns.size();
			printf("%d\n",sz);
			for(i=0;i<sz;i++){
				printf("%d\n",listAns[i]);
			}
		}else{
			//print semua edge yg di MST
			sort(listEdgeMst.begin(), listEdgeMst.end());
			int sz = listEdgeMst.size();
			printf("%d\n",sz);
			for(i=0;i<sz;i++){
				printf("%d\n", listEdgeMst[i]);
			}
		}
	}
	
	return 0;
};